...
class VerifyInterfaceDescriptionTestcase(aetest.Testcase):
    
    @aetest.test
    def verify_interface_description(self, device):
        '''
        verify interface counts with `show interfaces`
        '''

        try:
            # store execution result for later usage
            result = self.parameters[device].execute('show interfaces')

        except Exception as e:
            # abort/fail the testscript if show ip interface brief command
            # returns any exception such as connection timeout or command
            # failure
            self.failed('Device {} \'show interfaces \' failed: '
                            '{}'.format(device, str(e)),
                        goto = ['exit'])
        else:
            # extract ethernet interfaces
